...
train_generator = train_datagen.flow_from_directory('data/train', target_size=ts, batch_size=161, class_mode='binary')
validation_generator = test_datagen.flow_from_directory('data/validation', target_size=ts, batch_size=3, class_mode='binary')

# and now we fit the model to the data
model.fit_generator(train_generator, samples_per_epoch=3220, nb_epoch=10, validation_data=validation_generator, nb_val_samples=30)

# find the current date and time
dt = datetime.datetime.now()

# save the model, for use later in classifying notes
model.save('../../length/length_model-%s-%s-%s-%s-%s.h5' % (dt.minute, dt.hour, dt.day, dt.month, dt.year))
